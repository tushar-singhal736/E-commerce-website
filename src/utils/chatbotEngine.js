/**
 * SuperNova smart shopping assistant.
...
const buildWebsiteOverview = ({ products = [], settings = {}, discountPercent = 0, activeCoupon = "", coupons = [] }) => {
    const categories = getCategoryStats(products).map((c) => `${c.name} (${c.count})`).join(", ") || "loading";
    const pageList = Object.values(WEBSITE_PAGES)
        .map((p) => `${p.label}: ${p.desc}`)
        .join(" | ");
    return {
        reply: `${STORE_INFO.brand} ek full shopping website hai jahan fashion, shoes, jewellery aur electronics milte hain. Pages: ${pageList}. Categories: ${categories}. ${getOfferText(settings, discountPercent, activeCoupon, coupons)} Use search, filters, sorting, cart, wishlist, checkout, orders, profile aur support pages. Product search ke liye name ya category type karo, checkout ke liye login aur address fill karo, aur My Orders se order track karein.`,
        links: [
            { label: "Home", to: "/" },
...
const buildGreeting = ({ user, cart, sortedUserOrders, isAdmin, settings, discountPercent, activeCoupon, coupons = [] }) => {
    ...
    if (!user?.email) {
        return {
            reply: `Namaste! Main ${STORE_INFO.brand} ka shopping assistant hoon — bilkul Amazon help jaisa. Main products, prices, cart, checkout, orders, returns, coupons, policies aur support me guide kar sakta hoon. Login karne par aapke orders aur cart personal ho jayenge.`,
            links: [
                { label: "Browse products", to: "/products" },
...
const buildSmartReply = ({
    ...
    if (matchAny(q, ["cart", "bag", "basket", "subtotal"])) {
        if (!itemCount) {
            return {
                reply: "Cart abhi empty hai. Home ya Products page se Add to Cart karein.",
                links: [{ label: "Shop now", to: "/products" }],
            };
        }
        const auto = getAutoDiscountPercent(subtotal);
        return {
            reply: `Cart: ${itemCount} item(s), subtotal ${formatPrice(subtotal)}. Items: ${lines.join("; ")}. ${getOfferText(settings, discountPercent, activeCoupon, coupons)} Auto cart discount abhi ${auto}% ho sakta hai.`,
            links: [{ label: "Open cart", to: "/cart" }, { label: "Checkout", to: "/checkout" }],
        };
    }
    ...
    if (matchAny(q, ["coupon", "promo", "promocode", "code"])) {
        ...
        return {
            reply: `${activeText} Active coupons — ${detail} Cart page par code enter karke Apply dabayein.`,
    ...
    if (matchAny(q, ["checkout", "payment", "pay", "razorpay", "cod", "cash on delivery", "online pay"])) {
        if (!userEmail) {
            return {
                reply: "Checkout ke liye login zaroori hai. Uske baad address bhar kar Razorpay (UPI/card) ya Cash on Delivery choose kar sakte hain.",
                links: [{ label: "Login", to: "/login" }],
            };
        }
        return {
            reply: "Checkout flow: Cart → Checkout → address (name, phone, pincode, city, state) → payment method. Online: Razorpay secure payment. Offline: Cash on Delivery. Order success ke baad My Orders par track karein.",
    ...
    // —— Website overview and full site guide ——
    if (isWebsiteQuestion(q) || (q.includes("kaise") && !isLoginQuestion(q))) {
        return buildWebsiteOverview({ products, settings, discountPercent, activeCoupon, coupons });
    }
    ...
    // —— Stats ——
    if (matchAny(q, ["kitne product", "how many", "total product", "stock", "inventory", "count"])) {
        return {
            reply: `Catalog: ${products.length} products, ${categories.length} categories, ${totalStock} total stock. ${orders.length} store orders${isAdmin ? `, revenue ${formatPrice(orders.reduce((s, o) => s + getOrderTotal(o), 0))}` : ""}.`,
    ...
    return {
        reply: `Main ${STORE_INFO.brand} shopping assistant hoon. Main poore website ka overview, products, checkout, cart, orders, coupons, returns, policies aur support explain kar sakta hoon. Puchiye: "website guide", "site features", "product search", "track order", "coupon codes", "return policy", "contact support", ya kisi product ka naam. ${userEmail ? `Aap login ho (${userEmail}).` : "Login karne par personal orders bhi dikhenge."}`,
        links: [
            { label: "Products", to: "/products" },
...
export { STORE_INFO, WEBSITE_PAGES, buildSmartReply, buildLoginAnswer, isLoginQuestion, formatPrice, getOfferText };
